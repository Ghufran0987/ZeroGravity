...
{
    public class MealsSnacksLunchPageVmProvider : PageVmProviderBase, IMealsSnacksLunchPageVmProvider
    {
        private readonly IUserDataService _userDataService;
        private readonly IFastingDataService _fastingDataService;

        public MealsSnacksLunchPageVmProvider(ITokenService tokenService, IUserDataService userDataService, IFastingDataService fastingDataService) : base(tokenService)
        {
            _userDataService = userDataService;
            _fastingDataService = fastingDataService;
        }

        public async Task<ApiCallResult<MealDataProxy>> AddLunchAsync(MealDataProxy proxy, CancellationToken cancellationToken)
        {
        ...
        public async Task<ApiCallResult<MealDataProxy>> UpdateLunchAsync(MealDataProxy proxy, CancellationToken cancellationToken)
        {
            var dto = ProxyConverter.GetMealDataDto(proxy);
