...
{
    public class
        MealsSnacksLunchPageViewModel : VmBase<IMealsSnacksLunchPage, IMealsSnacksBreakfastPageVmProvider, MealsSnacksLunchPageViewModel>
    {
        ...
        public MealsSnacksLunchPageViewModel(IVmCommonService service, IMealsSnacksBreakfastPageVmProvider provider,
            ILoggerFactory loggerFactory, IApiService apiService, IEducationalInfoProvider _educationalInfoProvider) : base(service, provider, loggerFactory, apiService)
        {
            ...
            Meal = new MealDataProxy(MealSlotType.Lunch);
            FastingData = new EmptyFastingDataMessageProxy();
            MealsSnacksImageSource = ImageSource.FromResource("ZeroGravity.Mobile.Resources.Images.Meal_Lunch.png");
        }
        ...
        internal override void OnDailyCloseOverlay()
        {
            ...
            Service.HoldingPagesSettingsService.DoNotShowToday(HoldingPageType.Lunch);
        }
        ...
        public override async void OnNavigatedTo(INavigationParameters parameters)
        {
            ...
            { DateTime = DateTime.Today, SlotType = MealSlotType.Lunch, WithIngredients = true };

            var showOverlay = Service.HoldingPagesSettingsService.ShouldDailyShow(HoldingPageType.Lunch);
            if (showOverlay)
            {
                IsLoadingImageBusy = true;
                OpenOverlay();
                educationalInfoProvider.GetEducationalInfoByIdAsync(_cts.Token, StorageFolderConstants.Lunch).ContinueWith(async apiCallEducation =>
                {
            ...
            Provider.GetMealDataWithFilterAsync(filterMealDataDto, _cts.Token).ContinueWith(async result =>
            {
                Provider.GetPersonalGoalAsync(_cts.Token).ContinueWith(async apiCallResult =>
                {
                    ...
                    {
                        var mealExist = result.Result.Value.FirstOrDefault(m => m.MealSlotType == MealSlotType.Lunch);
                        if (mealExist != null)
                        {
                            Meal = mealExist;
                        ...
                        {
                            Meal = UpdateDefaultMealData(apiCallResult.Result.Value, MealSlotType.Lunch);
                        }
        ...
        protected override void OnCustomCloseOverlay()
        {
            ...
            Service.HoldingPagesSettingsService.DoNotShowAgain(HoldingPageType.Lunch);
        }
