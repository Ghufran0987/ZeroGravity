...
{
    public class MealsSnacksHealthySnackPageViewModel : VmBase<IMealsSnacksHealthySnackPage, IMealsSnacksBreakfastPageVmProvider, MealsSnacksHealthySnackPageViewModel>
    {
        ...
        public MealsSnacksHealthySnackPageViewModel(IVmCommonService service, IMealsSnacksBreakfastPageVmProvider provider,
            ILoggerFactory loggerFactory, IApiService apiService, IEducationalInfoProvider _educationalInfoProvider) : base(service, provider, loggerFactory, apiService)
        {
            ...
            Meal = new MealDataProxy(MealSlotType.HealthySnack, FoodAmountType.VeryLight);
            FastingData = new EmptyFastingDataMessageProxy();
            _cts = new CancellationTokenSource();
            MealsSnacksImageSource = ImageSource.FromResource("ZeroGravity.Mobile.Resources.Images.Meal_Healthy-Snack.png");
        }
        ...
        internal override void OnDailyCloseOverlay()
        {
            ...
            Service.HoldingPagesSettingsService.DoNotShowToday(HoldingPageType.HealthySnack);
        }
        ...
        public override async void OnNavigatedTo(INavigationParameters parameters)
        {
            ...
            { DateTime = DateTime.Today, SlotType = MealSlotType.HealthySnack, WithIngredients = true };

            var showOverlay = Service.HoldingPagesSettingsService.ShouldDailyShow(HoldingPageType.HealthySnack);
            if (showOverlay)
            {
                IsLoadingImageBusy = true;
                OpenOverlay();
                educationalInfoProvider.GetEducationalInfoByIdAsync(_cts.Token, StorageFolderConstants.HealthySancks).ContinueWith(async apiCallEducation =>
                {
            ...
            Provider.GetMealDataWithFilterAsync(filterMealDataDto, _cts.Token).ContinueWith(async result =>
            {
                Provider.GetPersonalGoalAsync(_cts.Token).ContinueWith(async apiCallResult =>
                {
                    ...
                    {
                        var mealExist = result.Result.Value.FirstOrDefault(m => m.MealSlotType == MealSlotType.HealthySnack);
                        if (mealExist != null)
                        {
                            Meal = mealExist;
                        ...
                        {
                            Meal = UpdateDefaultMealData(apiCallResult.Result.Value, MealSlotType.HealthySnack);
                        }
        ...
        protected override void OnCustomCloseOverlay()
        {
            ...
            Service.HoldingPagesSettingsService.DoNotShowAgain(HoldingPageType.HealthySnack);
        }
