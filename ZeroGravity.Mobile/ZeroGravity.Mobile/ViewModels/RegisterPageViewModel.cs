...
namespace ZeroGravity.Mobile.ViewModels
{
    ...
    [DataContract]
    public class RegisterPageViewModel : VmBase<IRegisterPage, IRegisterPageVmProvider, RegisterPageViewModel>
    {
        private CancellationTokenSource _cts;

        public RegisterPageViewModel(IVmCommonService service, IRegisterPageVmProvider provider,
            ILoggerFactory loggerFactory) : base(service, provider, loggerFactory)
        {
            Logger.LogInformation("Hello from RegisterPageViewModel!");
        ...
        private async void RegisterExecute()
        {
            ...
            {
                await Service.DialogService.DisplayAlertAsync(AppResources.Register_AcceptTermsRequired_Title, AppResources.Register_AcceptTermsRequired_Message, AppResources.Button_Ok);
                return;
            }

            IsBusy = true;

            var registerResult = await Provider.RegisterAsync(Email, Password, Password, AcceptTermsAndPrivacy, NewsletterSend, _cts.Token);

            if (registerResult.Success)
            {
                IsBusy = false;
                Logger.LogInformation($"User: {Email} successfully registered.");
                Email = string.Empty;
                Password = string.Empty;

                await Service.NavigationService.NavigateAsync(ViewName.RegisterSuccessPage);
            }
            else
            {
                IsBusy = false;

                if (registerResult.ErrorReason == ErrorReason.TaskCancelledByUserOperation)
                {
                    return;
                }
                ...
                {
                    await Service.DialogService.DisplayAlertAsync(AppResources.Register_Title, AppResources.Register_Error,
                        AppResources.Button_Ok);
                }
                else
                {
                    await Service.DialogService.DisplayAlertAsync(AppResources.Register_Title, registerResult.Message,
                        AppResources.Button_Ok);
                }
            }
        ...
        public override void OnNavigatedFrom(INavigationParameters parameters)
        {
            ...
            // in case user decides to NavigateFrom RegisterPage while login request task has not finished yet
