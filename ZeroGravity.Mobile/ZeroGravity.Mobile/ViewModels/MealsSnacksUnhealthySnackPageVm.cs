...
{
    public class MealsSnacksUnhealthySnackPageVm : VmBase<IMealsSnacksUnhealthySnackPage,
        IMealsSnacksBreakfastPageVmProvider, MealsSnacksUnhealthySnackPageVm>
    {
        ...
        public MealsSnacksUnhealthySnackPageVm(IVmCommonService service,
            IMealsSnacksBreakfastPageVmProvider provider, ILoggerFactory loggerFactory, IApiService apiService, IEducationalInfoProvider _educationalInfoProvider) :
            base(service, provider, loggerFactory, apiService)
        {
            ...
            Meal = new MealDataProxy(MealSlotType.UnhealthySnack, FoodAmountType.VeryLight);
            FastingData = new EmptyFastingDataMessageProxy();
            MealsSnacksImageSource = ImageSource.FromResource("ZeroGravity.Mobile.Resources.Images.Progress_UnhealthySnack_new.png");
        }
        ...
        internal override void OnDailyCloseOverlay()
        {
            ...
            Service.HoldingPagesSettingsService.DoNotShowToday(HoldingPageType.UnhealthySnack);
        }
        ...
        public override async void OnNavigatedTo(INavigationParameters parameters)
        {
            ...
            { DateTime = DateTime.Today, SlotType = MealSlotType.UnhealthySnack, WithIngredients = true };

            var showOverlay = Service.HoldingPagesSettingsService.ShouldDailyShow(HoldingPageType.UnhealthySnack);
            if (showOverlay)
            {
                IsLoadingImageBusy = true;
                OpenOverlay();
                educationalInfoProvider.GetEducationalInfoByIdAsync(_cts.Token, StorageFolderConstants.UnHealthySancks).ContinueWith(async apiCallEducation =>
                {
            ...
            Provider.GetMealDataWithFilterAsync(filterMealDataDto, _cts.Token).ContinueWith(async result =>
            {
                Provider.GetPersonalGoalAsync(_cts.Token).ContinueWith(async apiCallResult =>
                {
                    ...
                    {
                        var mealExist = result.Result.Value.FirstOrDefault(m => m.MealSlotType == MealSlotType.UnhealthySnack);
                        if (mealExist != null)
                        {
                            Meal = mealExist;
                        ...
                        {
                            Meal = UpdateDefaultMealData(apiCallResult.Result.Value, MealSlotType.UnhealthySnack);
                        }
        ...
        protected override void OnCustomCloseOverlay()
        {
            ...
            Service.HoldingPagesSettingsService.DoNotShowAgain(HoldingPageType.UnhealthySnack);
        }
